...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (changedPdf && pdfUrl) {
      ...
      const contentGen = await generateModuleContent(updated.id, updated.name, pdfUrl!, hasDescription).catch(() => ({ content: null, synopsis: null, description: null, aiError: "Erro ao gerar conteúdo" }));
      contentResult = contentGen.content;
      synopsisResult = contentGen.synopsis;
      if (contentGen.aiError) errors.push(contentGen.aiError);
    }

    return NextResponse.json({ module: updated, content: contentResult, synopsis: synopsisResult, aiError: errors.length > 0 ? errors.join("; ") : null });
  } catch (e) {
