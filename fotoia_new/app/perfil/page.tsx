...
type Session = { id: string; name: string; email: string; birthDate: string | null; role: string; authProvider: string };
...
export default function PerfilPage() {
  ...
  const [saving, setSaving] = useState<"name" | "birthDate" | "password" | null>(null);
        ...
        {editingBirthDate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => { if (!modalSuccess && !saving) { setEditingBirthDate(false); setBirthDate(session?.birthDate ? session.birthDate.slice(0, 10) : ""); } }}>
            <div className="w-full max-w-lg rounded-2xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-white">{modalSuccess ? "Sucesso" : "Alterar data de nascimento"}</h2>
              ...
              {modalSuccess ? (
                ...
                <form onSubmit={saveBirthDate} className="mt-5 space-y-4">
                  <div><label className="block text-base font-semibold text-zinc-200">Data de nascimento</label><input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} required disabled={saving === "birthDate"} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 text-base text-white outline-none transition focus:border-amber-500 disabled:opacity-50" /></div>
