# Retrato Imaginado — ANOTACOES DO APP
...
## Marca
- Nome correto: **Retrato Imaginado**
- Tema: fotografia, edição e criação de imagens com IA
- Paleta: violeta, azul e ciano em fundo escuro

---

## Infraestrutura
- Provedor oficial: **Contabo**, IP `37.60.227.11`
- URL: `https://retratoimaginado.cgialabs.com.br`
- SSH: `guilherme@37.60.227.11` por chave, sem senha armazenada
- Código: `/srv/apps/curso/fotoia_new`
- Container: `fotoia`, vinculado a `127.0.0.1:3000`
...
## Banco de Dados
- Banco: `fotoia`
- Usuário e senha: definidos somente no ambiente do servidor
- DATABASE_URL: definida somente no `.env` do servidor, nunca no repositório
...
- Build: `sudo docker compose -f /srv/stacks/sites/sites.compose.yaml build fotoia`
- Deploy: `sudo docker compose -f /srv/stacks/sites/sites.compose.yaml up -d fotoia`
- Logs: `sudo docker compose -f /srv/stacks/sites/sites.compose.yaml logs -f fotoia`
- Uploads persistentes: `/srv/data/fotoia/uploads`
...
# Build e deploy do Retrato Imaginado
sudo docker compose -f /srv/stacks/sites/sites.compose.yaml build fotoia
sudo docker compose -f /srv/stacks/sites/sites.compose.yaml up -d fotoia

# Logs
sudo docker compose -f /srv/stacks/sites/sites.compose.yaml logs -f fotoia
...
| Papel | Email | Senha |
|---|---|---|
| Aluno | `aluno.teste@fotoia.com` | Consulte o responsável |
| Professor | `professor.teste@fotoia.com` | Consulte o responsável |
| Admin | `admin@fotoia.com` | Consulte o responsável |
...
- ✔ Miniatura social exclusiva em `/logofotoia.jpg`
- Configurar uma cópia externa dos backups locais
- Rotacionar chaves de IA que tenham sido expostas anteriormente

---

## Correções 16/09/2026

### Bug: Dois modais ao editar módulo
- **Problema**: Ao clicar "Editar" abriam 2 modais — o antigo genérico (igual ao criar) e o novo com abas PDF/Vídeos
- **Causa**: `startEdit()` setava `setEditingId(m.id)` (abria overlay antigo) **e** `setEditModule` (abria novo)
- **Fix**: `setEditingId(null)` no `startEdit` — só o novo modal com abas abre

### Bug: PDF não gerava quiz/conteúdo ao substituir
- **Problema**: Substituir PDF e salvar não gerava quiz/descrição/resumo
- **Causa**: FormData enviava chave "pdf" mas API lia `form.get("file")`
- **Fix**: `fd.append("file", pdfFile)` em ModuleEditModal.tsx

### Bug: Excluir PDF dava erro
- **Problema**: Botão "Excluir PDF" enviava JSON `{pdfUrl: null}` mas API só aceitava FormData
- **Fix**: API PATCH /api/modules/[id] agora aceita JSON e FormData (route.ts)

### Mensagem de sucesso central (2s auto-close)
- Salvar módulo no editar: modal central que fecha em 2s (sem botão OK)
- Reorganizar ordem: mesma janela central com auto-close 2s

### Persistência de login (FotoIA)
- Login e register checam sessão em useEffect + evento pageshow (bfcache)

### Cabeçalho mobile (FotoIA)
- Logo, botões e nav ajustados para mobile

### Reorganização de módulos em modal
- Botão "Reorganizar módulos" abre modal com setas e salvar/cancelar

### Build limpo FotoIA
- Cache .next apagado e rebuild do zero (build anterior corrompido)

### Senhas de teste atualizadas para 13421342
- Regra: NUNCA mudar senhas sem autorização explícita
