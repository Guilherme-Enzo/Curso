...
const emptyForm: Form = { name: "", description: "", file: null };
...
export default function ModulesManager() {
  ...
  async function saveOrder() {
    ...
    setSavingOrder(true);
    try {
      const order = modules.map((m, i) => ({ id: m.id, order: i + 1 }));
      ...
      setStatus({ type: "ok", msg: "Ordem atualizada com sucesso!" });
      setReordering(false);
      load();
    } catch {
      setStatus({ type: "err", msg: "Falha de conexão" });
  ...
  async function sendForm(url: string, isCreate: boolean) {
    ...
    try {
      ...
      const contentMsg = data.content
        ...
        : "";
      return { message: quizMsg + contentMsg };
    } catch {
  ...
  async function handleCreate(_e: React.FormEvent) {
    const ok = await sendForm("/api/modules", true);
    if (!ok) return;
  ...
  async function handlePatch(id: string, _e: React.FormEvent) {
    const ok = await sendForm(`/api/modules/${id}`, false);
    if (!ok) return;
  ...
  async function handleRegenerate(m: CourseModule) {
    ...
    setStatus(null);
    try {
      const res = await fetch(`/api/modules/${m.id}/quiz`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
      ...
      <div className="space-y-3">
        ...
        {modules && modules.length > 1 && (
          <button
            ...
            className={`w-full rounded-2xl border-2 px-6 py-4 text-lg font-bold transition ${
              reordering
                ? "border-emerald-500/60 bg-emerald-500/10 text-emerald-400"
                : "border-dashed border-zinc-600 bg-zinc-900 text-zinc-400 hover:border-zinc-500 hover:text-zinc-300"
            }`}
          >
            {reordering ? "✓ Concluído" : "↕ Reorganizar módulos"}
          </button>
        )}
      </div>
      ...
      {reordering && modules && (
        <div className="rounded-2xl border border-zinc-700 bg-zinc-900 p-4">
          <p className="mb-3 text-sm text-zinc-400">
            Use as setas para reorganizar a ordem dos módulos. Clique em{" "}
            <span className="font-semibold text-emerald-400">Concluído</span> para
            salvar.
          </p>
          ...
          <button
            ...
            className="mt-4 w-full rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 py-3 text-base font-bold text-white transition hover:-translate-y-0.5 disabled:opacity-50"
          >
