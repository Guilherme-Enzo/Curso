...
function buildSynopsisPrompt(moduleName: string, text: string): string {
  return [
    ...
    `Com base no conteúdo abaixo, do módulo "${moduleName}" do livro "Injeção Eletrônica — Os Fundamentos",`,
...
function buildDescriptionPrompt(moduleName: string, text: string): string {
  return [
    ...
    "Exemplo: O Triplo C, Relação Estequiométrica, Central de Injeção Eletrônica (ECU), Tipos de Injeção Monoponto e Multiponto",
    "",
    "Regras:",
